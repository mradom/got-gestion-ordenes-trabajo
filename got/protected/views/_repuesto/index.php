...
$('.search-form form').submit(function(){
	$.fn.yiiGridView.update('repuesto-grid', {
		data: $(this).serialize()
	});
	return false;
});
...
<?php echo CHtml::link('Busqueda Avanzada','#',array('class'=>'search-button')); ?><div class="search-form" style="display:none">
...
<div class="right">
    <input id="add_repuesto2" type="button" style="display:block; clear: both;"
...
<?php
    ...
    array(
       'class' => 'CButtonColumn',
        'buttons' => array(
           'repuesto_delete' => array(
           'label' => Yii::t('admin_repuesto', 'Delete'), // text label of the button
            ...
            'options' => array("class" => "fan_del", 'title' => Yii::t('admin_repuesto', 'Delete')), // HTML options for the button   tag
            ),
           'repuesto_update' => array(
           'label' => Yii::t('admin_repuesto', 'Update'), // text label of the button
           'url' => '$data->id', // a PHP expression for generating the URL of the button
           ...
           'options' => array("class" => "fan_update", 'title' => Yii::t('admin_repuesto', 'Update')), // HTML options for the    button tag
              ),
           'repuesto_view' => array(
            'label' => Yii::t('admin_repuesto', 'View'), // text label of the button
            'url' => '$data->id', // a PHP expression for generating the URL of the button
            ...
            'options' => array("class" => "fan_view", 'title' => Yii::t('admin_repuesto', 'View')), // HTML options for the    button tag
              )
          ),
       'template' => '{repuesto_view}{repuesto_update}{repuesto_delete}',
            ),
    ),
 ...
 //VIEW
    ...
    $('.fan_view').each(function(index) {
        ...
        $(this).bind('click', function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/repuesto/returnView",
...
//UPDATE
    ...
    $('.fan_update').each(function(index) {
        ...
        $(this).bind('click', function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/Repuesto/returnForm",
...
// DELETE
    ...
    $('.fan_del').each(function(index) {
        ...
        deletes[id] = function() {
            $.ajax({
                type: "POST",
                url: "<?php echo Yii::app()->request->baseUrl;?>/repuesto/ajax_delete",
                data:{"id":id,"YII_CSRF_TOKEN":"<?php echo Yii::app()->request->csrfToken;?>"},
                    beforeSend : function() {
                    $("#repuesto-grid").addClass("ajax-sending");
                },
...
//CREATE 
    ...
    $('#add_repuesto2').bind('click', function() {
        $.ajax({
            type: "POST",
            url: "<?php echo Yii::app()->request->baseUrl;?>/Repuesto/returnForm",
